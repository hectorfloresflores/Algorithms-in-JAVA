...
public class PacificAtlanticWaterFlow {
    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        ...
        List<List<Integer>> finalValues = new LinkedList<>();
        boolean[][] possible = new boolean[heights.length][heights[0].length];
        for (int row = 0; row < heights.length; row++) {
            for (int column = 0; column < heights[0].length; column++) {
                Set<String> rowCol = new HashSet<>();
                boolean[] pacificAtlanticPrevValue = new boolean[2];
                bfs(heights, pacificAtlanticPrevValue, row, column, Integer.MAX_VALUE, rowCol, possible);

                if (pacificAtlanticPrevValue[0] && pacificAtlanticPrevValue[1]) {
                    finalValues.add(Arrays.asList(row, column));
                    possible[row][column] = true;
                }
    ...
    public void bfs(int[][] heights, boolean[] pacificAtlanticPrevValue, int row, int column, int prevValue, Set<String> rowCol, boolean[][] possible) {
        //Out of bounds!!!
        ...
        //Means root cell connect to both oceans!
        if ((pacificAtlanticPrevValue[0] && pacificAtlanticPrevValue[1]) || heights[row][column] > prevValue) {
            return;
        }

        if (rowCol.contains(String.format("%d:%d", row, column))) {
            return;
        }

        if (possible[row][column]) {
            pacificAtlanticPrevValue[0] = true;
            pacificAtlanticPrevValue[1] = true;
            return;
        }

        rowCol.add(String.format("%d:%d", row, column));
        //Reach Pacific Ocean
        if (row == 0 || column == 0) {
            pacificAtlanticPrevValue[0] = true;
        }
        //Reach Atlantic Ocean
        if (row == (heights.length - 1) || column == heights[0].length - 1) {
            pacificAtlanticPrevValue[1] = true;
        }

        //Avoid going back so lets block the opposite directions
            bfs(heights, pacificAtlanticPrevValue, row + 1, column, heights[row][column], rowCol, possible);
            bfs(heights, pacificAtlanticPrevValue, row - 1, column, heights[row][column], rowCol, possible);
            bfs(heights, pacificAtlanticPrevValue, row, column - 1, heights[row][column], rowCol, possible);
            bfs(heights, pacificAtlanticPrevValue, row, column + 1, heights[row][column], rowCol, possible);
    }
